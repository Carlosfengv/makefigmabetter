...
let gridStyles: DocumentGridStyleResource[] = [];
...
function dispatch(command: EditorCommand) {
  ...
  if (command.type === "flattenNodes") {
    if (!wasmDocument) { emitError(undefined, "TRANSIENT"); return; }
    const flattened = resolveFlattenNodesBatch(nodes, command.ids, command.vectorPath, createId, command.replacementId, {
      parentId: command.parentId,
      pageId: command.pageId,
      index: command.index,
    }, command.patch);
...
function dispatchTransaction(transaction: Extract<MainToWorker, { type: "transaction" }> ["transaction"]) {
  ...
  if (transaction.commands.length === 1 && transaction.commands[0].type === "flattenNodes") {
    const command = transaction.commands[0];
    const flattened = resolveFlattenNodesBatch(nodes, command.ids, command.vectorPath, createId, command.replacementId, {
      parentId: command.parentId,
      pageId: command.pageId,
