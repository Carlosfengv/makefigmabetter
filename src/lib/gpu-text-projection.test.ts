...
describe("GPU text projection", () => {
  ...
  it("uses shaped advances and font bearings to place line-local glyph quads", () => {
    const glyphs = projectGpuTextGlyphs({
      nodeId: "text", fontAssetId: "font", faceIndex: 0, variationAxesKey: "[]", fontSize: 20, pixelSize: 20,
    ...
    expect(glyphs?.[0]?.textureKey).toBe("font:0:[]:7:20");
  });
  ...
  it("keeps Variable Font instances in separate atlas identities", () => {
    const common = {
      ...
      x: 0, y: 0, rotation: 0, fill: "#000000", opacity: 1, lineHeight: 25,
      layout: { unitsPerEm: 1000, lines: [{ start: 0, end: 1, direction: "ltr" as const, advance: 500, glyphs: [{ glyphId: 7, cluster: 0, xAdvance: 500, yAdvance: 0, xOffset: 0, yOffset: 0 }] }] },
      rasters: new Map([[7, raster]]),
    };
    const thin = projectGpuTextGlyphs({ ...common, variationAxesKey: '[{"tag":"wght","value":100}]' });
    const bold = projectGpuTextGlyphs({ ...common, variationAxesKey: '[{"tag":"wght","value":800}]' });
  ...
  it("keeps Canvas as the fallback when any shaped glyph cannot be rasterized", () => {
    const base = { nodeId: "text", fontAssetId: "font", faceIndex: 0, variationAxesKey: "[]", fontSize: 20, pixelSize: 20, x: 0, y: 0, rotation: 0, fill: "#000000", opacity: 1, lineHeight: 25, layout: { unitsPerEm: 1000, lines: [{ start: 0, end: 1, direction: "ltr" as const, advance: 1, glyphs: [{ glyphId: 0, cluster: 0, xAdvance: 1, yAdvance: 0, xOffset: 0, yOffset: 0 }] }] } };
