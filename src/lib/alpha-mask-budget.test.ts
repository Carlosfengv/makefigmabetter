...
describe("alpha mask surface budget", () => {
  it("allows two nested full-resolution alpha compositions within the surface cap", () => {
    expect(admitAlphaMaskSurface(1920, 1080, 0)).toEqual({ accepted: true, bytes: 1920 * 1080 * 4 });
    expect(admitAlphaMaskSurface(1920, 1080, MAX_ALPHA_MASK_NESTING - 1)).toEqual({ accepted: true, bytes: 1920 * 1080 * 4 });
  });
  ...
  it("rejects a third nested mask and an over-budget surface", () => {
    ...
    const edge = Math.floor(Math.sqrt(MAX_ALPHA_MASK_SURFACE_BYTES / 4)) + 1;
    expect(admitAlphaMaskSurface(edge, edge, 0)).toMatchObject({ accepted: false, reason: "surface" });
  });
});
