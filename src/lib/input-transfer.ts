...
export function encodeInputBatch(events: readonly EditorInputEvent[]): ArrayBuffer {
  ...
  events.forEach((event, index) => {
    ...
    if (event.type === "pointer") {
      ...
      view.setUint8(offset + 2, (event.shiftKey ? 1 : 0) | (event.readOnly ? 2 : 0) | (event.altKey ? 4 : 0) | (event.drillDown ? 8 : 0) | (event.splitVectorSegment ? 16 : 0) | (event.deepSelect ? 32 : 0) | (event.ignoreConstraints ? 64 : 0));
...
export function decodeInputBatch(buffer: ArrayBuffer): EditorInputEvent[] | undefined {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (!Number.isFinite(x) || !Number.isFinite(y) || flags > (type === POINTER_KIND ? 127 : 1)) return undefined;
    if (type === POINTER_KIND) {
      ...
      events.push({ type: "pointer", event, x, y, shiftKey: (flags & 1) === 1, altKey: (flags & 4) === 4, button: view.getInt8(offset + 3), ...(flags & 2 ? { readOnly: true } : {}), ...(flags & 8 ? { drillDown: true } : {}), ...(flags & 16 ? { splitVectorSegment: true } : {}), ...(flags & 32 ? { deepSelect: true } : {}), ...(flags & 64 ? { ignoreConstraints: true } : {}), ...(occurredAt === undefined ? {} : { occurredAt }) });
      continue;
    }
    if (type === WHEEL_KIND) {
