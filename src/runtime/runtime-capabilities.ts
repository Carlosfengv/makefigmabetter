...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  ...
  },
  {
    ...
    surface: "write",
    status: "partial",
    limitation: "W12-L exposes fixed/hug Frame sizing, direct-child fill, physical min/max bounds, horizontal wrapping, per-track FILL distribution, counter-axis STRETCH, counter-axis HUG, horizontal baseline alignment, wrap-track spacing/distribution, Figma layoutAlign child overrides, absolute children, nested Frame reflow and MIN/CENTER/MAX/STRETCH/SCALE constraints through the transaction fence. Flexible children retain their sizing modes across atomic reparent and Undo; active Auto Layout owns flow geometry while preserving child constraints. Grid Auto Layout v1 uses semantics 56 and supports bounded FLEX/FIXED row and column tracks, independent gaps, deterministic row-major auto-flow, fixed-size containers and fixed/fill children through Core layout, Snapshot, Operation, REST import, WASM projection and Runtime. HUG tracks, manual cell placement, spans, track reordering, automatic row creation and Grid container HUG remain staged; text HUG still requires the richer text-auto-size API.",
  },
  {
