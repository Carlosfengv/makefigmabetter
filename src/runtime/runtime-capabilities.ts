...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  ...
  },
  {
    ...
    surface: "write",
    status: "partial",
    limitation: "Runtime creates local TextStyle and PaintStyle resources with Figma-compatible defaults, exposes them immediately through the pending catalog, and registers both through the Canonical transaction boundary so hash, history, snapshots and replay retain their identities. Local metadata, including independent plain and Markdown descriptions plus one HTTPS documentation link, every currently modeled TextStyle value field, and complete PaintStyle PaintStacks use explicit set-style commands; TextStyle letterSpacing retains the existing Canonical PIXELS-only limit. remove() requires full-document access, atomically clears every node/text-range consumer link while preserving materialized values, then uses explicit delete-style commands. Core rejects remote edits and direct deletion while references remain; set/delete participate in hash, history, Operation replay and Undo/Redo without overloading registration semantics. PERCENT TextStyle letter spacing and style Variable bindings remain staged.",
