...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  ...
  },
  {
    ...
    property: "ComponentNode.componentPropertyDefinitions|ComponentSetNode.componentPropertyDefinitions|addComponentProperty|editComponentProperty|deleteComponentProperty|componentPropertyValues|setProperties|swapComponent|detachInstance|removeOverrides|getMainComponentAsync",
    surface: "write",
    status: "partial",
    limitation: "Runtime reads Component and ComponentSet property definitions; combineAsVariants derives VARIANT defaults/options from Property=Value names, and REST import retains supported ComponentSet definitions as structured metadata. Runtime edits local Component definitions and projects add/rename/default/delete changes into linked local Instances in the same transaction. BOOLEAN, TEXT, INSTANCE_SWAP, VARIANT and SLOT definitions are admitted; INSTANCE_SWAP defaults must resolve to a live Component. ComponentSet property mutation, VariableAlias defaults, preferredValues and slotSettings are rejected, and SLOT/VARIANT deletion stays staged because Canonical cannot yet convert linked Slot nodes or variant structure without loss. Instance writes synchronously validate BOOLEAN, TEXT, INSTANCE_SWAP and VARIANT values against the main Component definition. swapComponent changes the source link, restores target non-SLOT defaults and clears direct overrides while retaining the existing subtree. detachInstance atomically replaces one bounded supported Instance subtree with fresh identities and returns the new Frame. Subtree reconciliation, nested-instance swap heuristics and property-reference authoring remain staged.",
  },
  {
