...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  {
    ...
    surface: "read",
    status: "partial",
    limitation: "M5 begins RevisionLease-frozen Smart Animate layer plans; exact Figma variant override-preservation heuristics and the remaining raster export surface stay staged.",
  },
  {
    id: "node.create-runtime",
    editorTypes: ["figma"],
    documentAccess: RUNTIME_DOCUMENT_ACCESS_MODES,
    nodeTypes: ["FRAME", "GROUP", "SECTION", "COMPONENT", "COMPONENT_SET", "INSTANCE", "SLOT", "SLICE", "RECTANGLE", "ELLIPSE", "POLYGON", "STAR", "VECTOR", "LINE", "TEXT", "CONNECTOR", "SHAPE_WITH_TEXT"],
    property: "createFrame|createGroup|createSection|createComponent|createComponentFromNode|combineAsVariants|ComponentNode.createInstance|ComponentNode.createSlot|createSlice|createRectangle|createEllipse|createPolygon|createStar|createVector|createLine|createText|createConnector|createShapeWithText",
    surface: "write",
    status: "partial",
    limitation: "Runtime synchronously creates the listed nodes with immediate read-your-writes projection and one fenced transaction. Local Component creation initializes a stable local key and complete local publishable metadata; createComponentFromNode atomically replaces a same-session Frame or Group outside Component/ComponentSet/Instance ancestry, preserving its direct children, geometry, page and final layer position. combineAsVariants wraps one or more local Components from ordinary same-page Page/Frame/Section parents in a new ComponentSet, preserves world geometry and document order, and derives variant values from Figma's Property=Value names. ComponentNode.createInstance clones a bounded supported subtree with durable source links and default component properties; ComponentNode.createSlot atomically creates one Frame-like Slot child, its unique SLOT component-property definition, and source-linked Slot children in existing local instances; Slice creation initializes a paint-free export region. Existing ComponentSet children, Auto Layout or structural source parents, remote Components, unsupported conversion sources, editor-specific and resource-backed creators remain staged.",
  },
  {
    id: "node.sync-write",
    editorTypes: ["figma"],
  ...
  },
  {
    ...
    property: "ComponentNode.componentPropertyDefinitions|ComponentSetNode.componentPropertyDefinitions|ComponentSetNode.defaultVariant|InstanceNode.mainComponent|InstanceNode.scaleFactor|InstanceNode.componentProperties|InstanceNode.exposedInstances|InstanceNode.isExposedInstance|variantProperties|componentPropertyReferences|addComponentProperty|editComponentProperty|deleteComponentProperty|componentPropertyValues|SlotNode.limitViolations|SlotNode.resetSlot|setProperties|swapComponent|detachInstance|removeOverrides|resetOverrides|getMainComponentAsync",
    surface: "write",
    status: "partial",
    limitation: "Runtime reads Component and ComponentSet property definitions; ComponentSet.defaultVariant returns the top-left local Component, while Instance.componentProperties returns official descriptor-shaped type/value/preferred/bound-variable records and componentPropertyValues remains as the earlier raw-value extension. Instance.scaleFactor accepts a bounded positive uniform factor and atomically rescales the root size plus descendant geometry, affine translations, strokes, corners, effects, Auto Layout metrics, editable vector coordinates, connector/table metadata, pixel Repeat offsets, and pixel-based text metrics without constraint cascades; subtree rebuilds reapply the retained factor. combineAsVariants derives VARIANT defaults/options from Property=Value names, and REST import retains supported ComponentSet definitions as structured metadata. Local ComponentSets can add VARIANT definitions and rename them within the official API boundary; renaming a child variant Component also recomputes the set options after rejecting incomplete or duplicate tuples. Every variant name, group definition and linked Instance value changes in one bounded transaction, while VARIANT deletion and direct default edits remain rejected as required by the pinned Plugin typings. Component and Instance variantProperties project the selected tuple. Moving or deleting the last Component dissolves its empty ComponentSet in Core, and PendingProjection mirrors that removal synchronously (including an empty Group ancestor) without emitting a duplicate delete command. Instance.setProperties resolves that tuple to exactly one sibling Component, preserves the Instance root identity and layout, rebuilds the selected source subtree, retains compatible custom property values, and reapplies direct override fields when source and target descendants have one deterministic type/name structure match; unmatched overrides are dropped rather than attached to the wrong layer. Runtime edits BOOLEAN, TEXT, INSTANCE_SWAP and SLOT definitions on local Components and ComponentSets, synchronizing ComponentSet custom definitions across every local variant, linked local Instance values and referenced sublayers in the same bounded transaction. INSTANCE_SWAP/SLOT preferredValues and complete bounded SlotSettings round-trip through metadata; Slot.limitViolations reports child-count and preferred-value violations from live contents, while resetSlot atomically replaces an Instance Slot's override subtree with fresh source-linked copies of the source Slot contents. slotContentId references can bind a direct child Frame to an existing SLOT property: Runtime replaces the Frame with a fresh Slot identity, preserves its child identities, and performs the same conversion on source-linked local Instances in one bounded transaction. Deleting a SLOT property restores source Frames, resets linked Slot overrides from the source contents, and then restores linked Frames atomically. A nested source Instance inside a local Component can set isExposedInstance; Runtime synchronizes every source-linked clone in one bounded transaction, exposes them through the linked root Instance's exposedInstances projection, and rejects writes on inherited clones. BOOLEAN/TEXT/INSTANCE_SWAP VariableAlias defaults preserve binding identity and resolve against inherited modes; variable resource, collection-mode and explicit subtree-mode changes atomically recompute definitions, non-overridden Instance defaults and referenced sublayers under the 10,000-node synchronous budget. Instance.setProperties also accepts VariableAlias values for BOOLEAN, TEXT and INSTANCE_SWAP properties, persists per-instance binding identity through compatible renames and component swaps, exposes it through componentProperties and boundVariables.componentProperties, and recomputes the instance and referenced sublayers when variables or inherited modes change; assigning a literal unbinds that property, while VARIANT and SLOT aliases are rejected. BOOLEAN visibility, TEXT characters and INSTANCE_SWAP mainComponent references are extension-backed, validated against their containing Component, cloned into Instances and applied atomically by setProperties; mainComponent reference changes preserve the nested Instance root and rebuild its descendants from the selected Component. createInstance preflights exposed nested swaps and materializes the selected Component descendants directly, so discarded source descendants never cross the Core transaction boundary. INSTANCE_SWAP defaults must resolve to a live Component. Instance.mainComponent is synchronously readable only under full-document access; assigning a live same-session Component keeps the Instance root identity and layout, atomically rebuilds source-linked descendants, restores target defaults and clears direct overrides. swapComponent uses the same bounded rebuild but preserves compatible property values and direct non-structural override fields only when source and target descendants have one deterministic type/name path; unmatched overrides are dropped. removeOverrides and its deprecated resetOverrides alias rebuild the current main Component subtree, restore component-property defaults and source visual fields, and clear direct overrides in one bounded transaction. detachInstance atomically replaces the outermost bounded Instance subtree with fresh identities, converts the requested nested Instance and every Instance ancestor to Frames, preserves unrelated nested Instances and returns the Frame corresponding to the requested node.",
  },
  {
  ...
  },
  {
    ...
    editorTypes: ["figma"],
    documentAccess: RUNTIME_DOCUMENT_ACCESS_MODES,
    property: "createTextStyle|createPaintStyle|getStyleById|getStyleByIdAsync|getLocalTextStyles|getLocalTextStylesAsync|getLocalPaintStyles|getLocalPaintStylesAsync|BaseStyle.name|BaseStyle.description|BaseStyle.descriptionMarkdown|BaseStyle.documentationLinks|BaseStyle.remove|TextStyle.fontSize|TextStyle.fontName|TextStyle.textDecoration|TextStyle.letterSpacing|TextStyle.lineHeight|TextStyle.leadingTrim|TextStyle.paragraphIndent|TextStyle.paragraphSpacing|TextStyle.textWrapStyle|TextStyle.listSpacing|TextStyle.hangingPunctuation|TextStyle.hangingList|TextStyle.textCase|TextStyle.boundVariables|TextStyle.setBoundVariable|PaintStyle.paints|PaintStyle.boundVariables",
    surface: "write",
    status: "partial",
    limitation: "Runtime creates local TextStyle and PaintStyle resources with Figma-compatible defaults, exposes them immediately through the pending catalog, and registers both through the Canonical transaction boundary so hash, history, snapshots and replay retain their identities. Local metadata, including independent plain and Markdown descriptions plus one HTTPS documentation link, every currently modeled TextStyle value field, PIXELS/PERCENT TextStyle letter spacing, and complete PaintStyle PaintStacks use explicit set-style commands. TextStyle supports typed Variable bindings for fontFamily, fontSize, fontStyle, fontWeight, letterSpacing, lineHeight, paragraphSpacing and paragraphIndent through semantics 53/local Snapshot v67. PaintStyle supports COLOR Variable aliases on Solid paints and individual gradient stops through semantics 54/local Snapshot v68, returns the aggregate aliases from PaintStyle.boundVariables.paints, and preserves per-paint aliases on Paint reads. Both style kinds materialize the default-mode value; variable or collection edits recompute bound values atomically, direct style-value writes unlink the affected binding, and bound variables or collections cannot be removed. Applying a percent-spaced TextStyle materializes its tracking from the style font size while retaining the style link. remove() requires full-document access, atomically clears every node/text-range consumer link while preserving materialized values, then uses explicit delete-style commands. Core rejects remote edits and direct deletion while references remain; set/delete participate in hash, history, Operation replay and Undo/Redo without overloading registration semantics.",
  ...
  },
  {
    ...
    property: "createPolygon|createStar|createVector|createLine|pointCount|innerRadius|vectorPaths|vectorNetwork|setVectorNetworkAsync|handleMirroring|strokeWeight|strokeCap|strokeJoin|strokeMiterLimit|dashPattern|union|subtract|intersect|exclude|booleanOperation|flatten",
    surface: "write",
    status: "partial",
    limitation: "W12-G supports Polygon and Star parameters, bounded Figma SVG VectorPaths, independent directed cubic VectorNetwork chains/cycles, and bounded shared-vertex branches with multiple globally styled fill regions. Vector and Highlight share the complete Runtime VectorLike read/write surface; Highlight preserves Canonical paths, exact extension-backed topology, node-level handleMirroring and mixed per-vertex handle reads. Branch loops materialize as closed subpaths and remaining edges as deterministic open subpaths for Canvas/SVG while exact topology survives through a path-bound versioned extension; any generic path edit invalidates that projection. Branches require NONE endpoint caps and no corner/join-local data; loop segments must form directed closed chains, may belong to only one loop, and all regions share one node-level winding rule and Paint Stack. setVectorNetworkAsync preserves tangents and shared start/end caps across ordinary open subpaths; region-local paints/styles, mixed region winding rules, per-vertex corners and mixed effective joins are rejected before staging. Stroke weight/join/miter/dash writes, concrete strokeCap writes, Figma mixed reads for asymmetric endpoint caps, and four live Vector Boolean operations pass the Worker fence. Boolean operands may come from ordinary same-page Page, Frame, Section or local Component parents; document-order operands and world transforms survive the atomic move. Active Auto Layout sources/targets, direct Group/Boolean source dissolution, Instance descendants and remote components are rejected. Flatten accepts one confirmed live Vector Boolean or one confirmed Vector/Rectangle/Ellipse/Polygon/Star/Line, an ordinary same-page target parent and final index; it preserves world transform, paints, mask state and a forced replacement identity in one undoable Core transaction. Boolean empty Rust results are supported. Flattening multi-node lists, Auto Layout/structural-source moves, region-local network styles, and degenerate Star ranges remain staged.",
  },
  {
    ...
    property: "layoutMode|layoutWrap|primaryAxisSizingMode|counterAxisSizingMode|layoutSizingHorizontal|layoutSizingVertical|minWidth|maxWidth|minHeight|maxHeight|primaryAxisAlignItems|counterAxisAlignItems|counterAxisSpacing|counterAxisAlignContent|layoutPositioning|layoutAlign|constraints|padding|itemSpacing|gridRowCount|gridColumnCount|gridRowGap|gridColumnGap|gridRowSizes|gridColumnSizes|gridAutoTracks|gridItemsPositioning|gridRowAnchorIndex|gridColumnAnchorIndex|gridRowSpan|gridColumnSpan|gridChildHorizontalAlign|gridChildVerticalAlign|appendChildAt|reorderRows|reorderColumns",
    surface: "write",
    status: "partial",
    limitation: "W12-L exposes fixed/hug Frame sizing, direct-child fill, physical min/max bounds, horizontal wrapping, per-track FILL distribution, counter-axis STRETCH, counter-axis HUG, horizontal baseline alignment, wrap-track spacing/distribution, Figma layoutAlign child overrides, absolute children, nested Frame reflow and MIN/CENTER/MAX/STRETCH/SCALE constraints through the transaction fence. Flexible children retain their sizing modes across atomic reparent and Undo; active Auto Layout owns flow geometry while preserving child constraints. Grid Auto Layout uses semantics 56 for bounded FLEX/FIXED tracks, semantics 57 for content-measured HUG tracks, semantics 58 for bounded row-auto-flow spans, semantics 59 for persistent manual anchors, semantics 60 for derived automatic rows, semantics 61 for per-child cell alignment and semantics 62 for physically mapped container HUG; it supports independent gaps, deterministic first-fit occupancy, fixed or intrinsically resolvable HUG containers, fixed/fill children outside HUG axes, appendChildAt, and atomic row/column track reorder with span closure through Core layout, Snapshot, Operation, REST import, WASM projection and Runtime. FLEX tracks remain invalid on a container-HUG axis to avoid a circular extent; text HUG still requires the richer text-auto-size API.",
  },
  {
