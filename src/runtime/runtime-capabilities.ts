...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  ...
  },
  {
    ...
    surface: "write",
    status: "partial",
    limitation: "W12-G supports Polygon and Star parameters, bounded Figma SVG VectorPaths, independent directed cubic VectorNetwork chains/cycles with one global region, and bounded open shared-vertex branches. Open branches materialize as deterministic segment subpaths for Canvas/SVG while their exact topology survives through a path-bound versioned extension; any generic path edit invalidates that projection. Branches require NONE endpoint caps and no region/corner/join-local data. setVectorNetworkAsync preserves tangents and shared start/end caps across ordinary open subpaths; region-local paints/styles, branched fill regions, per-vertex corners and mixed effective joins are rejected before staging. Stroke weight/join/miter/dash writes, concrete strokeCap writes, Figma mixed reads for asymmetric endpoint caps, and four live Vector Boolean operations pass the Worker fence. Boolean operands may come from ordinary same-page Page, Frame, Section or local Component parents; document-order operands and world transforms survive the atomic move. Active Auto Layout sources/targets, direct Group/Boolean source dissolution, Instance descendants and remote components are rejected. Flatten accepts one confirmed live Vector Boolean or one confirmed Vector/Rectangle/Ellipse/Polygon/Star/Line, an ordinary same-page target parent and final index; it preserves world transform, paints, mask state and a forced replacement identity in one undoable Core transaction. Boolean empty Rust results are supported. Flattening multi-node lists, Auto Layout/structural-source moves, filled branching networks, region-local network styles, and degenerate Star ranges remain staged.",
  },
  {
