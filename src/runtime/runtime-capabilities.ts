...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  ...
  },
  {
    ...
    surface: "write",
    status: "partial",
    limitation: "Runtime reads Component and ComponentSet property definitions; combineAsVariants derives VARIANT defaults/options from Property=Value names, and REST import retains supported ComponentSet definitions as structured metadata. Runtime edits BOOLEAN, TEXT, INSTANCE_SWAP and SLOT definitions on local Components and ComponentSets, synchronizing ComponentSet custom definitions across every local variant, linked local Instance values and referenced sublayers in the same bounded transaction. INSTANCE_SWAP/SLOT preferredValues and complete bounded SlotSettings round-trip through metadata; Slot.limitViolations reports child-count and preferred-value violations from live contents. BOOLEAN visibility, TEXT characters and INSTANCE_SWAP mainComponent references are extension-backed, validated against their containing Component, cloned into Instances and applied atomically by setProperties. INSTANCE_SWAP defaults must resolve to a live Component. Structural VARIANT mutation and VariableAlias defaults are rejected, and SLOT deletion stays staged because Canonical cannot yet convert linked Slot nodes without loss. swapComponent changes the source link, restores target non-SLOT defaults and clears direct overrides while retaining the existing subtree. detachInstance atomically replaces one bounded supported Instance subtree with fresh identities and returns the new Frame. Full swapped-subtree reconciliation, nested-instance swap heuristics and Slot reference authoring remain staged.",
  },
  {
