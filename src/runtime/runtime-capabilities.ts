...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  ...
  },
  {
    ...
    surface: "write",
    status: "partial",
    limitation: "W12-L exposes fixed/hug Frame sizing, direct-child fill, physical min/max bounds, horizontal wrapping, per-track FILL distribution, counter-axis STRETCH, counter-axis HUG, horizontal baseline alignment, wrap-track spacing/distribution, Figma layoutAlign child overrides, absolute children, nested Frame reflow and MIN/CENTER/MAX/STRETCH/SCALE constraints through the transaction fence. Flexible children retain their sizing modes across atomic reparent and Undo; active Auto Layout owns flow geometry while preserving child constraints. Grid Auto Layout uses semantics 56 for bounded FLEX/FIXED tracks, semantics 57 for content-measured HUG tracks and semantics 58 for bounded row-auto-flow spans; it supports independent gaps, deterministic first-fit occupancy, fixed-size containers and fixed/fill children outside HUG axes through Core layout, Snapshot, Operation, REST import, WASM projection and Runtime. Manual cell placement, track reordering, automatic row creation, child cell alignment and Grid container HUG remain staged; text HUG still requires the richer text-auto-size API.",
  },
  {
