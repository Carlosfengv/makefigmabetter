...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  ...
  },
  {
    ...
    editorTypes: ["figma"],
    documentAccess: RUNTIME_DOCUMENT_ACCESS_MODES,
    property: "createTextStyle|createPaintStyle|getStyleById|getStyleByIdAsync|getLocalTextStyles|getLocalTextStylesAsync|getLocalPaintStyles|getLocalPaintStylesAsync|BaseStyle.name|BaseStyle.description|BaseStyle.descriptionMarkdown|BaseStyle.remove|TextStyle.fontSize|TextStyle.fontName|TextStyle.textDecoration|TextStyle.letterSpacing|TextStyle.lineHeight|TextStyle.leadingTrim|TextStyle.paragraphIndent|TextStyle.paragraphSpacing|TextStyle.textWrapStyle|TextStyle.listSpacing|TextStyle.hangingPunctuation|TextStyle.hangingList|TextStyle.textCase|PaintStyle.paints",
    surface: "write",
    status: "partial",
    limitation: "Runtime creates local TextStyle and PaintStyle resources with Figma-compatible defaults, exposes them immediately through the pending catalog, and registers both through the Canonical transaction boundary so hash, history, snapshots and replay retain their identities. Local metadata, every currently modeled TextStyle value field, and complete PaintStyle PaintStacks use explicit set-style commands; TextStyle letterSpacing retains the existing Canonical PIXELS-only limit. remove() requires full-document access, atomically clears every node/text-range consumer link while preserving materialized values, then uses explicit delete-style commands. Core rejects remote edits and direct deletion while references remain; set/delete participate in hash, history, Operation replay and Undo/Redo without overloading registration semantics. Documentation links, PERCENT TextStyle letter spacing and style Variable bindings remain staged.",
