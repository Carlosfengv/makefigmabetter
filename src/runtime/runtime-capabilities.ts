...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  ...
  },
  {
    ...
    surface: "write",
    status: "partial",
    limitation: "W12-G supports Polygon and Star parameters, bounded Figma SVG VectorPaths, and the lossless VectorNetwork subset made of independent directed cubic chains/cycles with one global region. setVectorNetworkAsync preserves tangents and shared start/end caps across open subpaths; it rejects branches, direction conflicts, region-local paints/styles, per-vertex corners and mixed effective joins before staging a write. Stroke weight/join/miter/dash writes, concrete strokeCap writes, Figma mixed reads for asymmetric endpoint caps, and four live Vector Boolean operations pass the Worker fence. Boolean operands may come from ordinary same-page Page, Frame, Section or local Component parents; document-order operands and world transforms survive the atomic move. Active Auto Layout sources/targets, direct Group/Boolean source dissolution, Instance descendants and remote components are rejected. Flatten accepts one confirmed live Vector Boolean, an ordinary same-page target parent and final index; it preserves world transform and a forced replacement identity, supports empty Rust results, and is one undoable Core transaction. Flattening arbitrary node lists, Auto Layout/structural-source moves, fully general shared-vertex VectorNetwork topology, and degenerate Star ranges remain staged.",
  },
  {
