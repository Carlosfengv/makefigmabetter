...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  ...
  },
  {
    ...
    surface: "write",
    status: "partial",
    limitation: "Document-owned variable collections, modes, BOOLEAN/COLOR/FLOAT/STRING values, scopes and aliases persist through semantics 48/local Snapshot v63. Runtime reads filter local resources, preserve remote identity, and resolve bounded alias chains through the consumer's inherited mode. Scene nodes persist versioned width, height, characters, opacity, visible, uniform/per-side stroke weight, Auto Layout gap/padding, and min/max size binding identities in their forward-compatible extension map. Resolved values use the ordinary Core geometry/text/layout/appearance mutation paths, and direct property writes unlink only the corresponding bindings. Explicit modes inherit through ancestors; set/clear preflights and atomically recomputes up to 10,000 bound nodes in the affected subtree while respecting closer descendant overrides. Corner/grid fields, paint/effect bindings, EASING/TIMING, library import and catalog mutation remain staged.",
    errorCode: "RESOURCE_UNAVAILABLE",
  },
  {
