...
export const RUNTIME_CAPABILITIES: readonly RuntimeCapability[] = [
  ...
  },
  {
    ...
    editorTypes: ["figma"],
    documentAccess: RUNTIME_DOCUMENT_ACCESS_MODES,
    property: "variables.getVariableByIdAsync|variables.getVariableCollectionByIdAsync|variables.getLocalVariablesAsync|variables.getLocalVariableCollectionsAsync|variables.createVariableAlias|variables.setBoundVariableForPaint|variables.setBoundVariableForEffect|Variable.resolveForConsumer|boundVariables|setBoundVariable|effects|explicitVariableModes|resolvedVariableModes|setExplicitVariableModeForCollection|clearExplicitVariableModeForCollection",
    surface: "write",
    status: "partial",
    limitation: "Document-owned variable collections, modes, BOOLEAN/COLOR/FLOAT/STRING values, scopes and aliases persist through semantics 48/local Snapshot v63. Runtime reads filter local resources, preserve remote identity, and resolve bounded alias chains through the consumer's inherited mode. Scene nodes persist versioned width, height, characters, opacity, visible, corner radius, uniform/per-side stroke weight, Auto Layout gap/padding, min/max size, bounded Solid fill/stroke color, and bounded Drop/Inner Shadow plus Normal Layer/Background Blur binding identities in their forward-compatible extension map. Resolved values use the ordinary Core geometry/text/layout/paint/effect/appearance mutation paths, and direct property writes unlink only the corresponding bindings. Explicit modes inherit through ancestors; set/clear preflights and atomically recomputes up to 10,000 bound nodes in the affected subtree while respecting closer descendant overrides. Progressive/noise/texture/glass/shader effects, gradient-stop/text-range/style paint bindings, grid fields, EASING/TIMING, library import and catalog mutation remain staged.",
    errorCode: "RESOURCE_UNAVAILABLE",
  },
  {
