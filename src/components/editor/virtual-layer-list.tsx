...
import {
  layerKeyboardTarget,
  type LayerNavigationKey,
} from "@/lib/layer-keyboard-navigation";
...
export function VirtualLayerList({
  ...
  const renamingRef = useRef<{ id: string; draft: string } | undefined>(
    undefined,
  );
  ...
  const treeNodes = useMemo(
    () => layerTreeRows(nodes, collapsedIds),
    [nodes, collapsedIds],
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      top + LAYER_ROW_HEIGHT > element.scrollTop + element.clientHeight
    ) {
      element.scrollTop = Math.max(
        0,
        top - Math.max(0, (element.clientHeight - LAYER_ROW_HEIGHT) / 2),
      );
    }
  ...
  const moveKeyboardFocus = (
    ...
    if (event.key === "ArrowLeft") {
      ...
      const parent =
        node.parentId &&
        treeNodes.find((candidate) => candidate.node.id === node.parentId);
      if (parent) {
        event.preventDefault();
        ...
        requestAnimationFrame(() =>
          listRef.current
            ?.querySelector<HTMLButtonElement>(
              `button[data-layer-id="${parent.node.id}"]`,
            )
            ?.focus(),
        );
      }
    ...
    if (event.key === "ArrowRight") {
      ...
      const child =
        hasChildren && index >= 0
          ? treeNodes
              .slice(index + 1)
              .find((candidate) => candidate.depth === depth + 1)
          : undefined;
      if (child) {
        event.preventDefault();
        ...
        requestAnimationFrame(() =>
          listRef.current
            ?.querySelector<HTMLButtonElement>(
              `button[data-layer-id="${child.node.id}"]`,
            )
            ?.focus(),
        );
      }
    ...
    if (
      !(["ArrowUp", "ArrowDown", "Home", "End"] as const).includes(
        event.key as LayerNavigationKey,
      )
    )
      return;
    const target = layerKeyboardTarget(
      renderedLayerIds,
      node.id,
      event.key as LayerNavigationKey,
    );
    ...
    requestAnimationFrame(() =>
      listRef.current
        ?.querySelector<HTMLButtonElement>(`button[data-layer-id="${target}"]`)
        ?.focus(),
    );
  };
  const focusLayerRow = (id: string) =>
    requestAnimationFrame(() =>
      listRef.current
        ?.querySelector<HTMLButtonElement>(`button[data-layer-id="${id}"]`)
        ?.focus(),
    );
  const finishRename = (
    node: CanvasNode,
    ...
    requestAnimationFrame(() =>
      listRef.current
        ?.querySelector<HTMLInputElement>(
          `input[data-layer-rename="${node.id}"]`,
        )
        ?.focus(),
    );
    return true;
    ...
    rows.push(
      ...
      >
        {hasChildren && (
          <button
            type="button"
            className="absolute z-10 grid h-8 w-5 place-items-center rounded-sm text-muted-foreground outline-none hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring"
            style={{
              transform: `translateY(${virtualIndex * LAYER_ROW_HEIGHT}px)`,
              left: `${8 + depth * 16}px`,
            }}
        ...
        {renaming?.id === node.id ? (
          <Input
            data-layer-rename={node.id}
            className="absolute inset-x-0 h-8 rounded-md text-sm ring-2 ring-ring"
            style={{
              transform: `translateY(${virtualIndex * LAYER_ROW_HEIGHT}px)`,
              paddingLeft: `${hasChildren ? 28 + depth * 16 : 12 + depth * 16}px`,
            }}
            autoFocus
            maxLength={100}
            ...
            onBlur={() => finishRename(node, true, false)}
            onKeyDown={(event) => {
              if (event.key === "Enter") {
                event.preventDefault();
                finishRename(node, true, true);
              }
              if (event.key === "Escape") {
                event.preventDefault();
                finishRename(node, false, true);
              }
            }}
          />
        ) : (
          <button
            ...
            draggable={canEdit}
            className={cn(
              "absolute inset-x-0 grid h-8 grid-cols-[20px_minmax(0,1fr)_20px] items-center rounded-md px-2 text-left text-sm text-muted-foreground outline-none hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring",
            ...
            onKeyDown={(event) => {
              if (!beginRename(event, node)) moveKeyboardFocus(event, row);
            }}
            onDragStart={(event) => {
              if (!canEdit) {
                event.preventDefault();
            ...
            onDragOver={(event) => {
              ...
              setDropTarget({
                id: node.id,
                inside:
                  ["frame", "group", "section"].includes(node.kind) &&
                  middle > 0.25 &&
                  middle < 0.75,
              });
            }}
            onDrop={(event) => {
              ...
              if (id && id !== node.id)
                onDrop(
                  id,
                  inside ? { parentId: node.id } : { beforeId: node.id },
                );
          ...
          >
            <span className="grid w-5 place-items-center text-muted-foreground">
              {icon(node.kind)}
            </span>
  ...
  if (rows.length > MAX_LAYER_DOM_ROWS)
    throw new Error("Virtual layer list exceeded its DOM row budget");
  return (
    <div
      ref={listRef}
    ...
    >
      <div
        ...
        style={{ height: treeNodes.length * LAYER_ROW_HEIGHT }}
      >
        {rows}
      </div>
    </div>
