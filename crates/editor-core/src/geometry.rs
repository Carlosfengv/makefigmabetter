...
use clipper2_rust::{
    FillRule as ClipperFillRule, PathD as ClipperPath, PathsD as ClipperPaths,
    Point as ClipperPoint, difference_d, intersect_d, union_subjects_d, xor_d,
};
...
pub fn flatten_vector_path(
    ...
    for subpath in &path.subpaths {
        ...
        for index in 0..segment_count {
            ...
            let remaining =
                MAX_VECTOR_FLATTENED_POINTS.saturating_sub(all_points.len() + points.len());
        ...
        subpaths.push(FlattenedVectorSubpath {
...
fn normalized_operand_paths(
    path: &VectorPath,
    tolerance: f64,
) -> Result<ClipperPaths, GeometryError> {
...
pub fn vector_path_contains(
    ...
    for subpath in flattened.subpaths.iter().filter(|subpath| subpath.closed) {
        let Some(value) = polygon_winding(point, &subpath.points) else {
            return Ok(true);
        };
        winding += value;
    }
...
pub fn vector_path_stroke_mesh_with_caps(
    ...
    for subpath in flattened.subpaths {
        ...
        let mesh = stroke_mesh_for_polyline_with_caps(
            &subpath.points,
            style,
            start_cap,
            end_cap,
...
pub fn vector_path_dashed_stroke_mesh(
    ...
    for subpath in flattened.subpaths {
        ...
        let mesh =
            stroke_mesh_for_dashed_polyline(&subpath.points, style, dash_pattern, subpath.closed)?;
        triangles.extend(mesh.triangles);
    }
    let bounds = Bounds::from_points(&triangles.iter().flatten().copied().collect::<Vec<_>>());
    Ok(StrokeMesh { triangles, bounds })
...
pub fn outline_stroke_mesh(mesh: StrokeMesh) -> Result<FlattenedVectorPath, GeometryError> {
    ...
    let point_count = outlined
        .subpaths
        .iter()
        .map(|subpath| subpath.points.len())
        .sum::<usize>();
    if outlined.subpaths.len() > MAX_OUTLINE_VECTOR_SUBPATHS
        || point_count > MAX_OUTLINE_VECTOR_POINTS
    {
        return Err(GeometryError::ResourceLimit);
    }
    Ok(outlined)
...
pub fn stroke_mesh_for_dashed_polyline(
    ...
    if closed
        && runs.len() > 1
        && runs[0].first() == clean.first()
        && runs.last().and_then(|run| run.last()) == clean.first()
    {
...
pub fn stroke_meshes_for_per_side_rectangle_with_dash(
    ...
    let sides = [
        (
            top,
            [Point { x: 0.0, y: top_y }, Point { x: width, y: top_y }],
        ),
        (
            right,
            [
                Point { x: right_x, y: 0.0 },
                Point {
                    x: right_x,
                    y: height,
                },
...
fn continuous_rounded_rectangle_points(
    width: f64,
    height: f64,
    radii: [f64; 4],
    smoothing: f64,
    ...
    continuous_rounded_rectangle_corner(
        &mut points,
        width - top_right,
        top_right,
        top_right,
...
fn continuous_rounded_rectangle_corner(
...
fn add_join(
    ...
    if style.join == StrokeJoinStyle::Miter {
        if let Some(miter) =
            offset_line_intersection(previous_outer, previous.tangent, next_outer, next.tangent)
        {
...
fn add_semicircle(
    ...
    for index in 0..ROUND_STROKE_SEGMENTS / 2 {
        ...
        let right =
            start + std::f64::consts::PI * (index + 1) as f64 / (ROUND_STROKE_SEGMENTS / 2) as f64;
        push_triangle(
            triangles,
            center,
            Point {
                x: center.x + radius * left.cos(),
                y: center.y + radius * left.sin(),
            },
            Point {
                x: center.x + radius * right.cos(),
                y: center.y + radius * right.sin(),
            },
        )?;
    }
    Ok(())
...
fn push_triangle(
    ...
    if signed_area.abs() > 1e-12 {
        // Canvas combines the complete mesh into one non-zero fill so
        // translucent strokes do not darken where segment quads and join fans
        // overlap. A stable winding is therefore part of the mesh contract:
        // opposite-wound triangles cancel and expose circular holes at every
        // round join, most visibly on flattened closed curves.
        triangles.push(if signed_area > 0.0 {
            [a, b, c]
        } else {
            [a, c, b]
        });
...
fn offset_line_intersection(
    ...
    let point = Point {
        x: first.x + first_direction.x * amount,
        y: first.y + first_direction.y * amount,
    };
...
fn flatten_with_limit(
    ...
    if depth >= 24 || deviation <= tolerance {
        ...
        if output.len().saturating_add(needed) > maximum_points {
            return false;
        }
        if output.last().copied() != Some(from) {
            output.push(from);
...
fn nearest_on_curve(
    ...
    let (from, to) = match curve {
        Bezier::Line { from, to }
        | Bezier::Quadratic { from, to, .. }
        | Bezier::Cubic { from, to, .. } => (from, to),
    };
    ...
    if nearest
        .map(|current| candidate.distance < current.distance - 1e-12)
        .unwrap_or(true)
    {
...
fn polygon_winding(point: Point, polygon: &[Point]) -> Option<i32> {
    ...
    for (from, to) in polygon
        ...
        .take(polygon.len())
    {
        if point_on_segment(point, from, to, 1e-9) {
            return None;
        }
        if from.y <= point.y {
...
mod tests {
    ...
    fn parametric_outlines_are_top_anchored_clockwise_and_bounded() {
        let polygon =
            parametric_shape_outline(100.0, 80.0, ParametricShape::Polygon { point_count: 5 })
                .unwrap();
        assert_eq!(polygon.len(), 5);
        assert_point_near(polygon[0], p(50.0, 0.0));
        assert!(
            cross(polygon[0], polygon[1], polygon[2]) > 0.0,
            "Canvas-space winding must be clockwise"
        );

        let star = parametric_shape_outline(
            100.0,
            80.0,
            ParametricShape::Star {
                point_count: 5,
    ...
    fn vector_path_flattens_relative_handles_and_combines_fill_rules() {
        let triangle = crate::VectorPath {
            ...
            subpaths: vec![crate::VectorSubpath {
                closed: true,
                points: vec![
                    crate::VectorPoint {
                        id: crate::PointId(1),
                        position: p(0.0, 0.0),
                        handle_in: None,
                        handle_out: Some(p(25.0, 30.0)),
                        point_type: crate::VectorPointType::Corner,
                    },
                    crate::VectorPoint {
                        id: crate::PointId(2),
                        position: p(100.0, 0.0),
                        handle_in: Some(p(-25.0, 30.0)),
                        handle_out: None,
                        point_type: crate::VectorPointType::Corner,
                    },
                    crate::VectorPoint {
        ...
        assert_eq!(
            flattened.bounds,
            Some(Bounds::new(p(0.0, 0.0), p(100.0, 100.0)).unwrap())
        );
        ...
        let stroke = vector_path_stroke_mesh(
            &triangle,
            0.25,
            StrokeStyle {
                width: 4.0,
        ...
        hole.subpaths.push(crate::VectorSubpath {
            closed: true,
            points: vec![
                crate::VectorPoint {
                    id: crate::PointId(4),
                    position: p(35.0, 40.0),
                    handle_in: None,
                    handle_out: None,
                    point_type: crate::VectorPointType::Corner,
                },
    ...
    fn vector_flattening_enforces_a_transient_point_budget() {
        ...
        assert_eq!(
            curve.flatten_with_limit(0.001, 2),
            Err(GeometryError::ResourceLimit)
        );
    }
    ...
    fn stroke_mesh_matches_round_butt_and_square_cap_extents() {
        ...
        let round = stroke_mesh_for_polyline(
            &points,
            StrokeStyle {
                width: 4.0,
                cap: StrokeCapStyle::Round,
        ...
        assert_eq!(
            round.bounds,
            Some(Bounds::new(p(-2.0, -2.0), p(12.0, 2.0)).unwrap())
        );
        assert!(round.contains(p(-1.5, 0.0)));
        assert!(!round.contains(p(-2.1, 0.0)));

        let butt = stroke_mesh_for_polyline(
            &points,
            StrokeStyle {
                width: 4.0,
                cap: StrokeCapStyle::Butt,
                join: StrokeJoinStyle::Bevel,
                miter_limit: 4.0,
            },
            false,
        )
        ...
        let square = stroke_mesh_for_polyline(
            &points,
            StrokeStyle {
                width: 4.0,
                cap: StrokeCapStyle::Square,
                join: StrokeJoinStyle::Bevel,
                miter_limit: 4.0,
            },
            false,
    ...
    fn stroke_mesh_and_outline_allow_independent_open_path_caps() {
        ...
        let outline = outline_vector_path_with_caps(
            &path,
            0.25,
            style,
            StrokeCapStyle::Round,
        ...
        assert!(
            outline
                .subpaths
                .iter()
                .all(|subpath| subpath.closed && subpath.points.len() >= 3)
        );
    }
    ...
    fn stroke_mesh_uses_miter_limit_and_round_join_union() {
        ...
        let miter = stroke_mesh_for_polyline(
            &points,
            StrokeStyle {
                width: 4.0,
                cap: StrokeCapStyle::Butt,
        ...
        let limited = stroke_mesh_for_polyline(
            &points,
            StrokeStyle {
                width: 4.0,
                cap: StrokeCapStyle::Butt,
        ...
        assert!(
            round
                .triangles
                .iter()
                .all(|[a, b, c]| { (b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x) > 1e-12 })
        );
    }

    #[test]
    fn stroke_mesh_canonicalizes_duplicate_points_and_closed_paths() {
        let duplicate = stroke_mesh_for_polyline(
            &[p(0.0, 0.0), p(0.0, 0.0), p(10.0, 0.0)],
            StrokeStyle {
                width: 2.0,
                cap: StrokeCapStyle::Round,
                join: StrokeJoinStyle::Round,
        ...
        .unwrap();
        let clean = stroke_mesh_for_polyline(
        ...
        let closed = stroke_mesh_for_polyline(
            &[
                p(0.0, 0.0),
                p(10.0, 0.0),
                p(10.0, 10.0),
    ...
    fn dashed_line_mesh_preserves_each_visible_cap_and_omits_terminal_gaps() {
        ...
        assert_eq!(
            mesh.bounds,
            Some(Bounds::new(p(-5.0, -5.0), p(97.0, 5.0)).unwrap())
        );
    ...
    fn dashed_line_mesh_rejects_an_unbounded_number_of_tiny_runs() {
        let error = stroke_mesh_for_dashed_line(
            1.0,
            StrokeStyle {
                width: 1.0,
                cap: StrokeCapStyle::Butt,
    ...
    fn rounded_rectangle_stroke_mesh_has_stable_outer_bounds_and_inner_hole() {
        ...
        assert_eq!(
            mesh.bounds,
            Some(Bounds::new(p(-4.0, -4.0), p(104.0, 64.0)).unwrap())
        );
    ...
    fn per_side_rectangle_meshes_share_alignment_and_paint_order_contract() {
        let inside = stroke_meshes_for_per_side_rectangle(
            100.0,
            80.0,
            [6.0, 2.0, 4.0, 8.0],
            PerSideStrokeAlign::Inside,
        ...
        assert_eq!(
            inside[0].bounds,
            Some(Bounds::new(p(0.0, 0.0), p(100.0, 6.0)).unwrap())
        );
        assert_eq!(
            inside[1].bounds,
            Some(Bounds::new(p(98.0, 0.0), p(100.0, 80.0)).unwrap())
        );
        assert_eq!(
            inside[2].bounds,
            Some(Bounds::new(p(0.0, 76.0), p(100.0, 80.0)).unwrap())
        );
        assert_eq!(
            inside[3].bounds,
            Some(Bounds::new(p(0.0, 0.0), p(8.0, 80.0)).unwrap())
        );

        let outside = stroke_meshes_for_per_side_rectangle(
            100.0,
            80.0,
            [6.0, 2.0, 4.0, 8.0],
            PerSideStrokeAlign::Outside,
        ...
        assert_eq!(
            outside[0].bounds,
            Some(Bounds::new(p(0.0, -6.0), p(100.0, 0.0)).unwrap())
        );
        assert_eq!(
            outside[1].bounds,
            Some(Bounds::new(p(100.0, 0.0), p(102.0, 80.0)).unwrap())
        );
        assert_eq!(
            outside[2].bounds,
            Some(Bounds::new(p(0.0, 80.0), p(100.0, 84.0)).unwrap())
        );
        assert_eq!(
            outside[3].bounds,
            Some(Bounds::new(p(-8.0, 0.0), p(0.0, 80.0)).unwrap())
        );
    }
    ...
    fn dashed_per_side_rectangle_keeps_each_edge_dash_cycle_and_alignment() {
        ...
        assert_eq!(
            meshes[0].bounds,
            Some(Bounds::new(p(0.0, -6.0), p(92.0, 0.0)).unwrap())
        );
        assert_eq!(
            meshes[1].bounds,
            Some(Bounds::new(p(100.0, 0.0), p(102.0, 72.0)).unwrap())
        );
        assert_eq!(
            meshes[2].bounds,
            Some(Bounds::new(p(8.0, 80.0), p(100.0, 84.0)).unwrap())
        );
        assert_eq!(
            meshes[3].bounds,
            Some(Bounds::new(p(-8.0, 8.0), p(0.0, 80.0)).unwrap())
        );
    }
    ...
    fn dashed_rounded_rectangle_uses_the_same_normalized_contour_as_solid_stroke() {
        ...
        let solid = stroke_mesh_for_rounded_rectangle_with_radii(
            100.0,
            60.0,
            [12.0, 24.0, 8.0, 16.0],
            style,
        ...
        .unwrap();
        let dashed = stroke_mesh_for_dashed_rounded_rectangle_with_radii(
            100.0,
            60.0,
            [12.0, 24.0, 8.0, 16.0],
            style,
        ...
        assert!(
            dashed
                .bounds
                .is_some_and(|bounds| bounds.min.x <= -4.0 && bounds.max.x >= 104.0)
        );
    ...
    fn continuous_rounded_rectangle_preserves_dash_and_rejects_invalid_smoothing() {
        ...
        assert_eq!(
            stroke_mesh_for_continuous_rounded_rectangle_with_radii(
                100.0,
                60.0,
                [12.0; 4],
                1.1,
    ...
    fn vector_boolean_clipping_is_deterministic_for_all_four_operations() {
        ...
        assert_eq!(
            bounds(BooleanOperation::Union),
            Bounds::new(p(0.0, 0.0), p(15.0, 10.0)).unwrap()
        );
        assert_eq!(
            bounds(BooleanOperation::Intersect),
            Bounds::new(p(5.0, 0.0), p(10.0, 10.0)).unwrap()
        );
        assert_eq!(
            bounds(BooleanOperation::Subtract),
            Bounds::new(p(0.0, 0.0), p(5.0, 10.0)).unwrap()
        );
        let exclude = boolean_vector_paths(
            BooleanOperation::Exclude,
        ...
        assert_eq!(
            exclude.bounds,
            Some(Bounds::new(p(0.0, 0.0), p(15.0, 10.0)).unwrap())
        );
    ...
    fn vector_boolean_clipping_ignores_open_and_zero_area_operands() {
        ...
        assert_eq!(
            union.bounds,
            Some(Bounds::new(p(0.0, 0.0), p(10.0, 10.0)).unwrap())
        );
    }
    ...
    fn vector_boolean_degenerate_fixture_is_repeatable_for_tangency_overlap_and_self_intersection()
    {
        ...
        assert!(
            boolean_vector_paths(BooleanOperation::Intersect, &[left.clone(), touching], 0.25)
                .unwrap()
                .subpaths
                .is_empty()
        );
        ...
        assert_eq!(
            ...
            .unwrap()
            .bounds,
            Some(Bounds::new(p(3.0, 3.0), p(7.0, 7.0)).unwrap())
        );

        let identical = boolean_vector_paths(
            BooleanOperation::Exclude,
        ...
        let first = boolean_vector_paths(
            BooleanOperation::Union,
            &[bow_tie.clone(), left.clone()],
            0.25,
        )
        .unwrap();
    ...
    fn decorative_arrow_meshes_point_outward_and_size_from_stroke_width() {
        ...
        let end =
            decorative_cap_mesh(DecorativeCapStyle::ArrowEquilateral, 100.0, 1.0, 3.0).unwrap();
        ...
        let start =
            decorative_cap_mesh(DecorativeCapStyle::TriangleFilled, 0.0, -1.0, 3.0).unwrap();
        let start_bounds = start.bounds.unwrap();
        assert!(
            (start_bounds.min.x + 12.0).abs() < 1e-9,
            "start min x {start_bounds:?}"
        );
        assert!(
            (start_bounds.max.x - 0.0).abs() < 1e-9,
            "start max x {start_bounds:?}"
        );
    }
    ...
    fn decorative_diamond_and_circle_span_full_marker_size() {
        let diamond =
            decorative_cap_mesh(DecorativeCapStyle::DiamondFilled, 50.0, 1.0, 4.0).unwrap();
    ...
    fn decorative_arrow_lines_barbs_are_thin_and_reach_backward() {
        ...
        assert!(
            bounds.max.x <= 100.0 + 1.0 + 1e-6,
            "barbs stay near the tip: {bounds:?}"
        );
        assert!(
            bounds.min.x < 100.0 - 4.0,
            "barbs reach backward: {bounds:?}"
        );
        assert!(!mesh.triangles.is_empty());
    }
    ...
    fn nearest_vector_segment_returns_the_original_cubic_parameter() {
        use crate::{
            FillRule as CoreFillRule, PointId, VectorPoint, VectorPointType, VectorSubpath,
        };
        ...
        let hit = nearest_vector_path_segment(&path, Point { x: 5.0, y: 7.5 }, 0.01, 1.0)
            .unwrap()
            .unwrap();
        ...
        assert_eq!(
            nearest_vector_path_segment(&path, Point { x: 5.0, y: 30.0 }, 0.01, 1.0).unwrap(),
            None
        );
    }
}
