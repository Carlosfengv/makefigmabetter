...
pub fn document_from_snapshot(
    ...
    for decoded in decoded_nodes.values() {
        ...
        document.seed_auto_layout(decoded.node.id, decoded.auto_layout.clone()).map_err(|_| SnapshotError::Invalid)?;
    }
...
fn node_from_proto(
    node: v1::SceneNode,
) -> Result<(PageId, Node, Option<AssetId>, Option<TextProperties>, AutoLayout), SnapshotError> {
    ...
    Ok((
        ...
        node.text_properties
            ...
            .transpose()?,
        node.auto_layout.map(auto_layout_from_proto).transpose()?.unwrap_or_default(),
    ))
}
...
fn effect_from_proto(effect: v1::Effect) -> Result<Effect, SnapshotError> {
    match effect.kind.ok_or(SnapshotError::Invalid)? {
        ...
        v1::effect::Kind::InnerShadow(shadow) => Ok(Effect::InnerShadow(InnerShadow { offset_x: shadow.offset_x, offset_y: shadow.offset_y, blur_radius: shadow.blur_radius, spread: shadow.spread, color: color_from_proto(shadow.color.ok_or(SnapshotError::Invalid)?)?, visible: shadow.visible })),
...
fn auto_layout_to_proto(value: &AutoLayout) -> v1::AutoLayout {
    v1::AutoLayout {
        mode: match value.mode { LayoutMode::None => v1::LayoutMode::None, LayoutMode::Horizontal => v1::LayoutMode::Horizontal, LayoutMode::Vertical => v1::LayoutMode::Vertical } as i32,
        padding_top: value.padding[0], padding_right: value.padding[1], padding_bottom: value.padding[2], padding_left: value.padding[3], item_spacing: value.item_spacing, wrap: value.wrap,
        primary_alignment: alignment_to_proto(value.primary_alignment) as i32, counter_alignment: alignment_to_proto(value.counter_alignment) as i32,
        primary_sizing: sizing_to_proto(value.primary_sizing) as i32, counter_sizing: sizing_to_proto(value.counter_sizing) as i32,
        min_width: value.min_width, max_width: value.max_width, min_height: value.min_height, max_height: value.max_height, absolute: value.absolute,
    }
}
fn alignment_to_proto(value: LayoutAlignment) -> v1::LayoutAlignment { match value { LayoutAlignment::Start => v1::LayoutAlignment::Start, LayoutAlignment::Center => v1::LayoutAlignment::Center, LayoutAlignment::End => v1::LayoutAlignment::End, LayoutAlignment::SpaceBetween => v1::LayoutAlignment::SpaceBetween } }
fn sizing_to_proto(value: LayoutSizing) -> v1::LayoutSizing { match value { LayoutSizing::Fixed => v1::LayoutSizing::Fixed, LayoutSizing::Hug => v1::LayoutSizing::Hug, LayoutSizing::Fill => v1::LayoutSizing::Fill } }
fn auto_layout_from_proto(value: v1::AutoLayout) -> Result<AutoLayout, SnapshotError> {
    let mode = match v1::LayoutMode::try_from(value.mode).map_err(|_| SnapshotError::Invalid)? { v1::LayoutMode::None => LayoutMode::None, v1::LayoutMode::Horizontal => LayoutMode::Horizontal, v1::LayoutMode::Vertical => LayoutMode::Vertical, v1::LayoutMode::Unspecified => return Err(SnapshotError::Invalid) };
    let alignment = |value| match v1::LayoutAlignment::try_from(value).map_err(|_| SnapshotError::Invalid)? { v1::LayoutAlignment::Start => Ok(LayoutAlignment::Start), v1::LayoutAlignment::Center => Ok(LayoutAlignment::Center), v1::LayoutAlignment::End => Ok(LayoutAlignment::End), v1::LayoutAlignment::SpaceBetween => Ok(LayoutAlignment::SpaceBetween), v1::LayoutAlignment::Unspecified => Err(SnapshotError::Invalid) };
    let sizing = |value| match v1::LayoutSizing::try_from(value).map_err(|_| SnapshotError::Invalid)? { v1::LayoutSizing::Fixed => Ok(LayoutSizing::Fixed), v1::LayoutSizing::Hug => Ok(LayoutSizing::Hug), v1::LayoutSizing::Fill => Ok(LayoutSizing::Fill), v1::LayoutSizing::Unspecified => Err(SnapshotError::Invalid) };
    Ok(AutoLayout { mode, padding: [value.padding_top, value.padding_right, value.padding_bottom, value.padding_left], item_spacing: value.item_spacing, wrap: value.wrap, primary_alignment: alignment(value.primary_alignment)?, counter_alignment: alignment(value.counter_alignment)?, primary_sizing: sizing(value.primary_sizing)?, counter_sizing: sizing(value.counter_sizing)?, min_width: value.min_width, max_width: value.max_width, min_height: value.min_height, max_height: value.max_height, absolute: value.absolute })
}
...
fn vector_path_from_proto(path: v1::VectorPath) -> Result<VectorPath, SnapshotError> {
    let fill_rule = match v1::FillRule::try_from(path.fill_rule).map_err(|_| SnapshotError::Invalid)? { v1::FillRule::NonZero => FillRule::NonZero, v1::FillRule::EvenOdd => FillRule::EvenOdd, v1::FillRule::Unspecified => return Err(SnapshotError::Invalid) };
    let subpaths = path.subpaths.into_iter().map(|subpath| Ok(VectorSubpath {
        closed: subpath.closed,
        points: subpath.points.into_iter().map(|point| {
            let point_type = match v1::VectorPointType::try_from(point.point_type).map_err(|_| SnapshotError::Invalid)? { v1::VectorPointType::Corner => VectorPointType::Corner, v1::VectorPointType::Mirrored => VectorPointType::Mirrored, v1::VectorPointType::Asymmetric => VectorPointType::Asymmetric, v1::VectorPointType::Unspecified => return Err(SnapshotError::Invalid) };
            let handle_in = match (point.handle_in_x, point.handle_in_y) { (None, None) => None, (Some(x), Some(y)) => Some(editor_core::geometry::Point { x, y }), _ => return Err(SnapshotError::Invalid) };
            let handle_out = match (point.handle_out_x, point.handle_out_y) { (None, None) => None, (Some(x), Some(y)) => Some(editor_core::geometry::Point { x, y }), _ => return Err(SnapshotError::Invalid) };
...
mod tests {
    use super::*;
    use editor_core::{
        AssetId, AssetReference, DEFAULT_PAGE_ID, DropShadow, Effect, NodeKind, Page, PageId, ParagraphStyle,
    ...
    fn wire_snapshot_round_trips_canonical_document() {
            ...
            .seed_node_on_page(
                ...
                Node {
                   ...
                   stroke_weights: Vec::new(),
                    ...
                    opacity: 1.0,
                    blend_mode: editor_core::BlendMode::Normal,
                    drop_shadow: Some(DropShadow { offset_x: 3.0, offset_y: 5.0, blur_radius: 9.0, spread: 1.0, color: Color::from_srgb_u8([15, 23, 42], 96), visible: true }),
                    effect_stack: vec![Effect::DropShadow(DropShadow { offset_x: 3.0, offset_y: 5.0, blur_radius: 9.0, spread: 1.0, color: Color::from_srgb_u8([15, 23, 42], 96), visible: true })],
                    corner_radius: 0.0,
        ...
        assert_eq!(restored.node(NodeId(7)).unwrap().drop_shadow.unwrap().blur_radius, 9.0);
    ...
    fn wire_snapshot_round_trips_a_live_boolean_operation_selector() {
        ...
        document.seed_node_on_page(DEFAULT_PAGE_ID, boolean).unwrap();

        let restored = document_from_wire_snapshot(&snapshot_from_document(&document, 1).unwrap()).unwrap();
        assert_eq!(
            restored.node(NodeId(7)).and_then(|node| node.boolean_operation),
    ...
    fn future_node_kind_is_rejected_as_requires_newer_client_not_corruption() {
        ...
        node.kind = 14; // beyond NODE_KIND_SLICE = 13
