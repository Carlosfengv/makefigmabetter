...
if ! [[ "$duration_seconds" =~ ^[0-9]+$ ]] || ! [[ "$cycle_seconds" =~ ^[0-9]+$ ]] || ! [[ "$command_timeout_seconds" =~ ^[1-9][0-9]*$ ]] || [[ "$cycle_seconds" -eq 0 ]]; then
  echo "Phase 2 professional stability duration and cycle must be whole seconds, and command timeout must be positive." >&2
  exit 64
fi
...
for attempt in $(seq 1 50); do
  # The installed Playwright CLI writes its accessibility snapshot to stdout;
  # `--filename` is not supported by this version.
  ...
  if grep -Fq "Engine worker online" "$evidence_dir/snapshot.txt" \
    ...
    printf 'Phase 2 professional composite stability fixture ready after %s snapshot attempt(s).\n' "$attempt" | tee "$evidence_dir/readiness.log"
    ready=1
    break
  fi
  sleep 0.2
...
while :; do
  ...
  capture_cycle_command performance "$evidence_dir/performance-run-$(printf '%04d' "$run").txt" "$pwcli" --session "$session" eval "$performance_probe"
  capture_cycle_command memory "$evidence_dir/memory-run-$(printf '%04d' "$run").txt" "$pwcli" --session "$session" eval "$memory_probe"
