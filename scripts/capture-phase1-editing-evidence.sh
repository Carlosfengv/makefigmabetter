...
for attempt in $(seq 1 50); do
  ...
  if grep -Fq "Rust/WASM bridge ready" "$evidence_dir/ready.snapshot.txt" \
    && grep -Fq "fixed Phase 1 render composite fixture loaded" "$evidence_dir/ready.snapshot.txt" \
    && grep -Fq "5/100000 nodes" "$evidence_dir/ready.snapshot.txt"; then
...
if ! grep -Fq "6/100000 nodes" "$evidence_dir/created.snapshot.txt" || ! grep -Fq "Rectangle" "$evidence_dir/created.snapshot.txt"; then
...
if ! grep -Fq "5/100000 nodes" "$evidence_dir/undo.snapshot.txt" || grep -Fq 'button "□ Rectangle ◉"' "$evidence_dir/undo.snapshot.txt"; then
...
if ! grep -Fq "6/100000 nodes" "$evidence_dir/redo.snapshot.txt" || ! grep -Fq 'button "□ Rectangle ◉"' "$evidence_dir/redo.snapshot.txt"; then
